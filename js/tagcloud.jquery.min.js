(function (h) {
    ...
    var e = {
        ...
        timer: 5,
        fontMultiplier: 15,
        hoverStyle: {border: "1px solid #935C26", color: "#935C26"},
        mouseOutStyle: {border: "none", color: "red"}
    };
    var i = {
        mouseOver: null,
        lastFy: null,
    ...
    function c() {
        ...
        if (Math.abs(o) > 0.01 || Math.abs(p) > 0.01) {
            ...
            while (j++ < k.tLength) {
                ...
                l[j].alpha = n / 2;
                l[j].css({
                    left: k.whratio * (l[j].x - l[j].w * n) + k.halfWidth,
                    top: k.hwratio * (l[j].y - l[j].h * n) + k.halfHeight,
                    opacity: l[j].alpha,
                    "font-size": m.fontMultiplier * l[j].alpha + "px",
                    "z-index": Math.round(-l[j].cz)
                })
            }
        }
    }
